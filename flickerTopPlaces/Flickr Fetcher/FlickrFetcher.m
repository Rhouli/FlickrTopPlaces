...
+ (NSString *)extractNameOfPlace:(id)placeId fromPlaceInformation:(NSDictionary *)place
{
    ...
    } else if ([placeId isEqualToString:[place valueForKey:FLICKR_PLACE_ID]]){
        NSArray* tmp = [[place valueForKeyPath:FLICKR_PLACE_NAME] componentsSeparatedByString:@","];
            name = tmp[0];
    }
...
{
    NSArray* names = [[place valueForKeyPath:FLICKR_PLACE_NAME] componentsSeparatedByString:@","];
    if ([names count] == 3)
        return names[1];
